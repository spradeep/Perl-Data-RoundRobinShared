# This Makefile is for the Data::RoundRobinShared extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Data/RoundRobinShared.pm]
#     AUTHOR => q[S Pradeep <pradeep@pradeep.net.in>]
#     NAME => q[Data::RoundRobinShared]
#     PREREQ_PM => { IPC::Shareable=>q[0.6] }
#     VERSION_FROM => q[lib/Data/RoundRobinShared.pm]
...
NAME_SYM = Data_RoundRobinShared
VERSION = 0.11
...
MAKE = make
FULLEXT = Data/RoundRobinShared
BASEEXT = RoundRobinShared
PARENT_NAME = Data
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Data/RoundRobinShared.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Data/RoundRobinShared.pm
...
TO_INST_PM = lib/Data/RoundRobinShared.pm

PM_TO_BLIB = lib/Data/RoundRobinShared.pm \
...
DISTNAME = Data-RoundRobinShared
DISTVNAME = Data-RoundRobinShared-0.11
...
manifypods : pure_all  \
	lib/Data/RoundRobinShared.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Data/RoundRobinShared.pm $(INST_MAN3DIR)/Data::RoundRobinShared.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Data-RoundRobinShared' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.11' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Serve data in a round robin manner, keeping the data in a shared memory so that it can be used by multiple processes and each get data in a roundrobin manner.' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - S Pradeep <pradeep@pradeep.net.in>' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    IPC::Shareable:  0.6' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Serve data in a round robin manner, keeping the data in a shared memory so that it can be used by multiple processes and each get data in a roundrobin manner.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>S Pradeep &lt;pradeep@pradeep.net.in&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="IPC::Shareable" VERSION="0.6" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Data/RoundRobinShared.pm blib/lib/Data/RoundRobinShared.pm 
